...
export default function EmployeePage() {
  ...
  const dayCards = useMemo<DayCard[]>(() => {
    return weekDays.map((day) => {
      ...
      return {
        ...
        periodId: myShift?.period_id ?? activeWeek?.default_period_id ?? null,
      };
    });
  }, [activeWeek?.default_period_id, allRestCounts, dayLimits, shifts, weekDays]);
  ...
  useEffect(() => {
    async function bootstrap() {
      ...
      setActiveWeek(weekResponse.data ?? null);
  ...
  useEffect(() => {
    ...
    void fetchMyShifts();
  }, [activeWeek, employeeName]);

  useEffect(() => {
    const channel = supabase
      .channel(`employee-sync-${activeWeek?.start_date ?? "no-week"}-${employeeName || "anon"}`)
      .on(
        "postgres_changes",
        {
          event: "*",
          ...
          filter: activeWeek && employeeName ? `week_start=eq.${activeWeek.start_date}` : undefined,
        },
        async (_payload: RealtimePostgresChangesPayload<EmployeeWeekShiftRow>) => {
          if (!activeWeek || !employeeName.trim()) return;
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: activeWeek ? `week_start=eq.${activeWeek.start_date}` : undefined,
        },
        async () => {
          if (!activeWeek) return;
  ...
  async function confirmSetRest() {
    if (!activeWeek || !employeeName.trim() || !pendingRestDay) return;
  ...
  return (
    <main className="page-container">
      ...
      <header className="page-header">
        ...
        <p>{activeWeek ? `本周范围：${formatWeekRange(activeWeek.start_date, activeWeek.end_date)}` : "暂无开放的排休周"}</p>
      </header>

      {message ? <div className="toast-pill">{message}</div> : null}

      <section className="calendar-grid">
        {loading && !activeWeek ? <div className="empty-state">数据加载中...</div> : null}
                  ...
                  <>
                    ...
                    {canSetRest ? (
                      <button
                        className="btn-rest"
                        type="button"
                        disabled={!activeWeek || isRestFull || submittingKey !== null}
